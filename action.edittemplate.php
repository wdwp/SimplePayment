...
if (isset($params['submit'])) {
	...
	if ($templatename != '' && $template != '') {
		...
		$params = array('tab_message' => $this->Lang('templateupdated'), 'active_tab' => 'templates');
		$this->Redirect($id, 'defaultadmin', $returnid, $params);
	} else {
...
$this->smarty->assign('templatename', $templatename);
$this->smarty->assign('prompt_template', $this->Lang('templatecontent'));
$this->smarty->assign('template', $this->CreateTextArea(false, $id, $template, 'template', '', '', '', '', 80, 25, 'class="defaultfocus"'));
